...
public struct NYCSchool: Codable, PropertyReflectable {
    ...
    func getAllKeys(hide: [HiddenCellType]) -> [String] {
        return  self.getAllCodingKeys { CodingKeys(stringValue: $0)!.stringValue }.filter { key in
            let keymatch = hide.filter { key1 in
                return key1.rawValue == key
            }
            if keymatch.count <= 0 {
